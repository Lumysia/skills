...
Use this skill to configure an agent host to load this skills repository and requested MCP servers without assuming one provider or host.
...
2. Read `references/workflow.md` before editing host config.
...
5. Add requested MCP servers using the host's native MCP shape.
6. Validate changed config with an available parser or schema, then report reload steps and any unresolved blockers.

Hard dependencies are target host, config scope, writable config destination, and permission to modify user-level config. Do non-destructive discovery first; ask one concise question only when a hard dependency remains unclear.

## Resources

- `references/workflow.md`: execution rules for discovery, config merges, MCP setup, validation, and handoff.
- `references/hosts.md`: host-specific examples and common config locations; verify against the active host before editing.
- `references/evals.md`: smoke scenarios for host targeting, MCP preservation, and validation.
